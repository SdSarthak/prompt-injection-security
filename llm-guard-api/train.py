...
def download_and_process_dataset(output_path: str = str(LOCAL_CSV_PATH), force_download: bool = False):
    """
    ...
    try:
        ...
        if 'prompt' not in df.columns or 'label' not in df.columns:
            raise ValueError(f"Dataset does not contain required columns after processing. Got: {df.columns.tolist()}")
...
def train_classifier(dataset_path: str, model_output_dir: str, epochs: int = 3, force_retrain: bool = False):
    """
